...
import { IQueryTransactionsAdminArgs, ITransactionsModel } from 'graphql/_server';
import TRANSACTIONS_ADMIN from 'graphql/queries/transactionsAdmin';
...
import AdminTransactionDetails from './AdminTransactionDetails/AdminTransactionDetails';
...
const AdminTransactions: FC = () => {
  ...
  useEffect(() => {
    if (data?.transactionsAdmin.items) {
      actions.setRows(data?.transactionsAdmin.items);
      actions.setTotal(data.transactionsAdmin.meta.total);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data?.transactionsAdmin.items, data?.transactionsAdmin.meta.total]);

  const refetchWithCache = useRefetchWithCache(['transactionsAdmin']);
  ...
  return (
    <Box>
      ...
      <UniversalTable
        rows={data?.transactionsAdmin?.items || []}
