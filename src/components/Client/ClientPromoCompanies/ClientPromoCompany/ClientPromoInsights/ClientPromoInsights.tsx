...
import {
  ...
  IQueryCompanyInsightsBuyerArgs
} from 'graphql/_server';
import COMPANY_INSIGHTS_BUYER from 'graphql/queries/companyInsightsBuyer';
...
const ClientPromoInsights: FC<{ companyId: number | undefined }> = ({ companyId }) => {
  ...
  useEffect(() => {
    if (data?.companyInsightsBuyer.items) {
      actions.setRows(data?.companyInsightsBuyer.items);
      actions.setTotal(data.companyInsightsBuyer.meta.total);
    }
  }, [actions, data?.companyInsightsBuyer.items, data?.companyInsightsBuyer.meta.total]);
  ...
  return (
    <Box sx={styles.tableWrapper}>
      ...
      <UniversalTable
        rows={data?.companyInsightsBuyer?.items || []}
...
export default ClientPromoInsights;
