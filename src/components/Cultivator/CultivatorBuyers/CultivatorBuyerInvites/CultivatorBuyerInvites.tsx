...
import CultivatorInviteUser from 'components/Cultivator/shared/CultivatorInviteUser/CultivatorInviteUser';
import Loader from 'components/Loader/Loader';

const CultivatorInvites: FC = () => {
  const { dataMe } = useMe();
  ...
  return (
    <Box>
        ...
        <Plugin>
          ...
          <DataTypeProvider
            formatterComponent={(params) => {
              return (
                <Link href={`/auth/join/buyer?code=${params.row?.code}`} target="_blank">
                  {params.row?.code}
                </Link>
              );
            }}
      ...
      <CultivatorInviteUser isOpen={isOpen} closeModal={closeModal} text="Buyer" />
    </Box>
  );

  function handleSetTab(newTab: string) {
    setTab(newTab);
    goTo(Routes.CULTIVATOR_BUYERS);
  }
};

export default CultivatorInvites;
