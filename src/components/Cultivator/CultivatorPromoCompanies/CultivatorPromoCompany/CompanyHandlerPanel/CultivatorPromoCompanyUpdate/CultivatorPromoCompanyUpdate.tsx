...
const CultivatorPromoCompanyUpdate: FC<{
      ...
      payload: {
        filters: [
          ...
          },
          {
            ...
            operation: FilterOperationEnum.NotEqual,
            type: FilterFieldTypeEnum.Number,
            value: (productsForm || [])?.map((item) => `${item.value}`)
          }
        ]
      }
    }
  ...
  const facilityProductsUm: TSelectOptions[] = useMemo(() => {
    const items = productsData?.productsCultivator?.items;
    if (items) {
    ...
    return [];
  }, [productsData?.productsCultivator?.items]);
  ...
  const stylesUm = useMemo(() => {
    return styles(isOnlyDatesEdit);
  }, [isOnlyDatesEdit]);

  return (
    <Dialog sx={stylesUm.dialog} maxWidth="sm" fullWidth={true} open={isOpen} onClose={closeModal}>
      <>
        <form onSubmit={handleSubmit(onSubmit)} onKeyDown={preventEnterOnKeyDown}>
          ...
          <Box sx={stylesUm.block}>
            <DialogTitle sx={stylesUm.title}>
              <Typography variant="h3">{dataCompany?.companyName}</Typography>
            </DialogTitle>
            <DialogContent>
              <IconButton sx={stylesUm.close} type="button" onClick={closeModal} size="small">
...
export default CultivatorPromoCompanyUpdate;
