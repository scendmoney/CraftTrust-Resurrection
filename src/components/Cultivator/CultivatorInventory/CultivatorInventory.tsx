...
import { IProductsModel, IQueryProductsCultivatorArgs } from 'graphql/_server';
...
import CultivatorInventoryProductWrapper from './CultivatorInventoryProduct/CultivatorInventoryProductWrapper';
...
const CultivatorInventory: FC = () => {
  ...
  useEffect(() => {
    if (data?.productsCultivator.items) {
      actions.setRows(data?.productsCultivator.items);
      actions.setTotal(data.productsCultivator.meta.total);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data?.productsCultivator.items, data?.productsCultivator.meta.total]);
  const refetchWithCache = useRefetchWithCache(['productsCultivator']);
      ...
      <Plugin>
        <DataTypeProvider
          formatterComponent={(props) => (
            <AvatarNameFormatter3 url={props?.row?.thumbnail?.url} name={props.row?.item?.name}>
              <MenuFormatter rowId={props.row.id} rowStatus={props.row.status} />
  ...
  return (
    <Box>
      ...
      <UniversalTable
        rows={data?.productsCultivator?.items || []}
      ...
      <SidebarBottom isOpen={Boolean(id)} close={clearQuery} isFullwidth>
        <CultivatorInventoryProductWrapper />
      </SidebarBottom>
    </Box>
  );
...
export default CultivatorInventory;
